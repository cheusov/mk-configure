...
\mode<presentation>
\title{MK-CONFIGURE (MK-C) -- lightweight easy to use replacement for GNU Autotools}
...
\begin{frame}
  ...
  \begin{block}{Design principles and goals}
    \begin{itemize}
    \item I detest code generation as in Autotools and CMake!\\
      Library approach is used instead.
    \item Written in bmake, portable version of NetBSD make(1),
      and UNIX tools. No heavy dependencies like python, ruby and perl.
      bmake is not as powerful as RuPyPe, but\\
      \h{\mbox{bmake+sh} is good enough} for this task.
    \item Basic principles are very close to the
    ...
    \item KISS.  mk-c consists of about 4000 lines of code.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Design principles and goals}
    ...
    \item mk-configure is not only for end-users and packagers
      ...
      So, one of the main goal is to provide a convenient \h{tool for
        development}.
    \item Declarative approach of writing Makefile(s). Build and
      installation process are controlled with a help of special
      variables and bmake's include files.
    \item Cross-compilation.
    \item Extensibility. Extensions to mk-configure are implemented
...
\begin{CodeNoLabel}
\prompt{\$ mkcmake}
/opt/SUNWspro/bin/\h{CC}    -c \h{-KPIC} foo.cc -o foo.os
/opt/SUNWspro/bin/\h{CC}    -c \h{-KPIC} bar.cc -o bar.os
/opt/SUNWspro/bin/\h{CC}    -c \h{-KPIC} baz.cc -o baz.os
...
\prompt{\$ ls -l}
total 4
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 dict
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 dictd
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 dictfmt
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 dictzip
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:03 doc
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 libcommon
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 libdz
drwxr-xr-x 2 cheusov users   1 ﬂÌ‚ 26 12:01 libmaa
-rw-r--r-- 1 cheusov users 306 ﬂÌ‚ 26 12:03 \h{Makefile}
\prompt{\$}
    \end{CodeNoLabel}
  \end{block}
\end{frame}
...
\begin{frame}[fragile,t]
  ...
  \item Building, installing, uninstalling, cleaning
    ...
    \h{uninstall},
    \h{clean}, \h{cleandir} (\h{distclean}), \h{installdirs} and others.
...
\begin{frame}[fragile,t]
  ...
  \item \h{Cross-building}. mk-configure itself doesn't run produced
...
\begin{frame}[fragile,t]
  ...
  \begin{block}{Packages in UNIX-like system and distributions}
    ...
    \begin{itemize}
    \item FreeBSD, NetBSD (\h{devel/bmake})
    ...
    \begin{itemize}
    \item FreeBSD, NetBSD (\h{devel/mk-configure})
    \item Debian/Ubuntu\\
      deb http://mova.org/\~{}cheusov/pub/debian lenny main\\
      deb-src http://mova.org/\~{}cheusov/pub/debian lenny main
    \end{itemize}
