...
# SUBPRJS specifies a dependency graph
# for all subprojects.

SUBPRJS=   libcommon:dict   # dict depends on libcommon
SUBPRJS+=  libcommon:dictd
SUBPRJS+=  libcommon:dictzip
SUBPRJS+=  libcommon:dictfmt
SUBPRJS+=  libdz:dictzip
SUBPRJS+=  libmaa:dict
...
...
%%   \frametitle{LMDBG: rather comples real life example}
%%   LMDBG is lightweight modular malloc debuger written in C, shell and AWK.
%%   It consists of numerous scripts and one LD\_PRELOAD'ed library written in C.
%%   \begin{enumerate}
%%   \end{enumerate}
%% \end{frame}
...
\begin{frame}[fragile,t]
  ...
  \item Support for projects with multiple subprojects
    (\ModuleName{mkc.subprjs.mk} and \ModuleName{mkc.subdir.mk}).
  \end{enumerate}
  \end{block}
\end{frame}
